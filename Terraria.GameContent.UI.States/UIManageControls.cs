	...
	private void SetupGamepadPoints(SpriteBatch spriteBatch)
	{
		...
		for (int num3 = 0; num3 < snapPoints.Count; num3++)
		{
			...
			if (snapPoints[num3].Name == "Thin")
			{
				...
				uILinkPoint7.Down = ((num3 < snapPoints.Count - 2) ? (key + 2) : num);
				uILinkPoint7.Up = ((num3 < 2) ? (num + 1) : ((snapPoints[num3 - 1].Name == "Wide") ? (key - 1) : (key - 2)));
				...
				if (num3 < snapPoints.Count)
				{
					...
					uILinkPoint8.Down = ((num3 >= snapPoints.Count - 1) ? num : ((snapPoints[num3 + 1].Name == "Wide") ? (key + 1) : (key + 2)));
					uILinkPoint8.Up = ((num3 < 2) ? (num + 1) : (key - 2));
					UILinkPointNavigator.Shortcuts.FANCYUI_HIGHEST_INDEX = key;
				}
			}
			else
			{
				...
				uILinkPoint9.Unlink();
				UILinkPointNavigator.SetPosition(key, snapPoints[num3].Position);
				uILinkPoint9.Down = ((num3 < snapPoints.Count - 1) ? (key + 1) : num);
				uILinkPoint9.Up = ((num3 < 1) ? (num + 1) : ((snapPoints[num3 - 1].Name == "Wide") ? (key - 1) : (key - 2)));
				UILinkPointNavigator.Shortcuts.FANCYUI_HIGHEST_INDEX = key;
				UILinkPointNavigator.Points[num].Up = key;
			}
		}
