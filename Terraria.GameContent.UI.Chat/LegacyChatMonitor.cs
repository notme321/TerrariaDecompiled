...
public class LegacyChatMonitor : IChatMonitor
{
	...
	{
		showCount = (int)((float)(Main.screenHeight / 3) / FontAssets.MouseText.Value.MeasureString("1").Y) - 1;
		switch (linesOffset)
		{
		case 1:
			startChatLine++;
	...
	public void NewTextMultiline(string text, bool force = false, Color c = default(Color), int WidthLimit = -1)
	{
		...
		List<List<TextSnippet>> list = ((WidthLimit == -1) ? Utils.WordwrapStringSmart(text, c, FontAssets.MouseText.Value, TextMaxLengthForScreen, 10) : Utils.WordwrapStringSmart(text, c, FontAssets.MouseText.Value, WidthLimit, 10));
	...
	public void DrawChat(bool drawingPlayerChat)
	{
		...
		for (int i = num; i < num2; i++)
		{
			if (drawingPlayerChat || (chatLine[i].showTime > 0 && chatLine[i].parsedText.Length != 0))
			{
				...
				ChatManager.DrawColorCodedStringWithShadow(Main.spriteBatch, FontAssets.MouseText.Value, chatLine[i].parsedText, new Vector2(88f, Main.screenHeight - 30 - 28 - num3 * 21), 0f, Vector2.Zero, Vector2.One, out hoveredSnippet);
