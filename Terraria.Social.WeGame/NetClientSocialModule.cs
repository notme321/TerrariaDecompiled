...
public class NetClientSocialModule : NetSocialModule
{
	...
	public unsafe void OnRailEvent(RAILEventID id, EventBase data)
	{
		...
		WeGameHelper.WriteDebugString("OnRailEvent,id=" + ((object)(*(RAILEventID*)(&id))/*cast due to .constrained prefix*/).ToString() + " ,result=" + ((object)System.Runtime.CompilerServices.Unsafe.As<RailResult, RailResult>(ref data.result)/*cast due to .constrained prefix*/).ToString());
		if ((int)id <= 12010)
		{
			if ((int)id != 12002)
			{
	...
	{
		WeGameHelper.WriteDebugString("OnRailSetMetaData - " + ((object)System.Runtime.CompilerServices.Unsafe.As<RailResult, RailResult>(ref ((EventBase)data).result)/*cast due to .constrained prefix*/).ToString());
	}
