...
public class NetServerSocialModule : NetSocialModule
{
	...
	private string DumpFriendListString(List<RailFriendInfo> list)
	{
		...
		{
			stringBuilder.AppendLine($"friend_id: {((RailComparableID)item.friend_rail_id).id_}, type: {item.friend_type}, online: {((object)System.Runtime.CompilerServices.Unsafe.As<EnumRailPlayerOnLineState, EnumRailPlayerOnLineState>(ref item.online_state.friend_online_state)/*cast due to .constrained prefix*/).ToString()}, playing: {item.online_state.game_define_game_playing_state}");
		}
	...
	public unsafe void OnRailEvent(RAILEventID event_id, EventBase data)
	{
		...
		WeGameHelper.WriteDebugString("OnRailEvent,id=" + ((object)(*(RAILEventID*)(&event_id))/*cast due to .constrained prefix*/).ToString() + " ,result=" + ((object)System.Runtime.CompilerServices.Unsafe.As<RailResult, RailResult>(ref data.result)/*cast due to .constrained prefix*/).ToString());
