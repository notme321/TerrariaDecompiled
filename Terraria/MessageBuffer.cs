	...
	public void GetData(int start, int length, out int messageType)
	{
		...
		case 12:
		{
			...
			if (Netplay.Clients[whoAmI].State == 3)
			{
				...
				bool flag5 = NetMessage.DoesPlayerSlotCountAsAHost(whoAmI);
				Main.countsAsHostForGameplay[whoAmI] = flag5;
				if (NetMessage.DoesPlayerSlotCountAsAHost(whoAmI))
				{
					NetMessage.TrySendData(139, whoAmI, -1, null, whoAmI, flag5.ToInt());
				}
		...
		case 17:
		{
			...
			if (b7 == 0)
			{
				WorldGen.KillTile(num176, num177, flag10);
			...
			if (b7 == 2)
			{
				WorldGen.KillWall(num176, num177, flag10);
			}
			if (b7 == 3)
			{
			...
			if (b7 == 4)
			{
				WorldGen.KillTile(num176, num177, flag10, effectOnly: false, noItem: true);
			}
			if (b7 == 5)
			{
		...
		case 20:
		{
			...
			for (int num232 = num228; num232 < num228 + num230; num232++)
			{
				for (int num233 = num229; num233 < num229 + num231; num233++)
				{
					...
					bool flag15 = bitsByte28[3];
					if (Main.netMode != 2)
					{
						tile4.liquid = (byte)(flag15 ? 1u : 0u);
					}
					...
					if (tile4.active())
					{
						...
						else if (!flag14 || tile4.type != type13)
						{
							tile4.frameX = -1;
							tile4.frameY = -1;
						}
		...
		case 30:
		{
			...
			Main.player[num137].hostile = flag9;
			if (Main.netMode == 2)
			{
		...
		case 65:
		{
			...
			case 2:
			{
				...
				NetMessage.TrySendData(65, -1, -1, null, 0, num51, vector.X, vector.Y, num52, flag3.ToInt(), num54);
				int num55 = -1;
				float num56 = 9999f;
			...
			{
				NetMessage.TrySendData(65, -1, whoAmI, null, num53, num51, vector.X, vector.Y, num52, flag3.ToInt(), num54);
			}
			break;
		}
		case 66:
		...
		case 71:
			if (Main.netMode == 2)
			{
				...
				NPC.ReleaseNPC(x13, y13, type11, style2, whoAmI);
			}
			break;
		case 72:
			if (Main.netMode == 1)
		...
		case 79:
		{
			...
			WorldGen.PlaceObject(x9, y9, type7, mute: false, style, num140, random, direction);
			if (Main.netMode == 2)
			{
				NetMessage.SendObjectPlacement(whoAmI, x9, y9, type7, style, num140, random, direction);
			}
			break;
		}
		case 80:
		...
		case 105:
			if (Main.netMode != 1)
			{
				...
				WorldGen.ToggleGemLock(i2, j2, flag2);
			}
			break;
		case 106:
			if (Main.netMode == 1)
		...
		case 117:
		{
			...
			if (Main.netMode != 2 || whoAmI == num234 || (Main.player[num234].hostile && Main.player[whoAmI].hostile))
			{
				...
				bool flag16 = bitsByte31[0];
				bool pvp2 = bitsByte31[1];
				int num236 = reader.ReadSByte();
				Main.player[num234].Hurt(playerDeathReason2, damage3, num235, pvp2, quiet: true, flag16, num236);
				if (Main.netMode == 2)
				{
					NetMessage.SendPlayerHurt(num234, playerDeathReason2, damage3, num235, flag16, pvp2, num236, -1, whoAmI);
				}
			}
			break;
		}
		...
		case 121:
		{
			...
			{
				tEDisplayDoll.ReadItem(num184, reader, flag12);
			}
			else
			{
				reader.ReadInt32();
			...
			if (Main.netMode == 2)
			{
				NetMessage.TrySendData(b, -1, num182, null, num182, num183, num184, flag12.ToInt());
			}
			break;
		}
		case 122:
		...
		case 124:
		{
			...
			{
				tEHatRack.ReadItem(num135, reader, flag8);
			}
			else
			{
				reader.ReadInt32();
			...
			if (Main.netMode == 2)
			{
				NetMessage.TrySendData(b, -1, num133, null, num133, num134, num135, flag8.ToInt());
			}
			break;
		}
		case 125:
