...
public class WorkshopHelper
{
	public class UGCBased
	{
		...
		public class PublishedItemsFinder
		{
			...
			private void OnSteamUGCQueryCompleted(SteamUGCQueryCompleted_t pCallback, bool bIOFailure)
			{
				...
				for (uint num = 0u; num < pCallback.m_unNumResultsReturned; num++)
				{
					...
					SteamWorkshopItem value = new SteamWorkshopItem
					{
						Title = ((SteamUGCDetails_t)(ref val)).m_rgchTitle,
						Description = ((SteamUGCDetails_t)(ref val)).m_rgchDescription
					};
		...
		public abstract class APublisherInstance
		{
			...
			private unsafe void CreateItemResult(CreateItemResult_t param, bool bIOFailure)
			{
				...
				{
					_issueReporter.ReportDelayedUploadProblemWithoutKnownReason("Workshop.ReportIssue_FailedToPublish_WithoutKnownReason", ((object)(*(EResult*)(&param.m_eResult))/*cast due to .constrained prefix*/).ToString());
					_endAction(this);
				}
			}
			...
			private unsafe void UpdateItemResult(SubmitItemUpdateResult_t param, bool bIOFailure)
			{
				//IL_0000: Unknown result type (might be due to invalid IL or missing references)
				//IL_0025: Unknown result type (might be due to invalid IL or missing references)
				//IL_0026: Unknown result type (might be due to invalid IL or missing references)
